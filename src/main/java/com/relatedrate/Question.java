    ...
    private boolean generateQuestion() throws IOException{
        ...
        for(int i = question.length() - 1; i >= 0; i--){
            if(question.charAt(i) == ':'){
                ...
                if(variables.containsKey(question.substring(colonSequence[0] + 1, colonSequence[1]))) variables.replace(question.substring(colonSequence[0] + 1, colonSequence[1]), (double) randomValue);
        ...
        equation.substituteValues(equation.getRoot(), variables);
        equation.printEquation(equation.getRoot());
        System.out.println();

        //preconditions.forEach((key, value) -> System.out.println(key + ": " + value));
